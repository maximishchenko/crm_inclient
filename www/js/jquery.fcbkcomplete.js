  ...
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function addItem(title, value, preadded, locked, focusme) {
        ...
        if(data_titles[value].length && $('li[rel="'+value+'"]').length<2){
                var li = $('<li class="'+liclass+'" rel="'+value+'" id="pt_'+id+'"></li>').prepend(data_titles[value]).append(aclose);
          ...
          if (maximum) {
            if (options.filter_selected && element.children('option[value="' + object.key + '"]').hasClass("selected")) {
              //nothing here...
               ...
               avatar = '';
               if (data_avatar[object.key].length) {
                avatar = '<img class="miniAvatar" src="' + data_avatar[object.key] + '" alt="">';     
               }
               
              content += '<li rel="' + object.key  + '">' + xssDisplay(itemIllumination(object.value, etext)) + '<div style="float:right">'+ 
              avatar  +
