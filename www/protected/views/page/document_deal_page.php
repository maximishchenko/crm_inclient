        ...
        <nav class="clients-nav_01 navbar">
            <ul class="nav navbar-nav">
                ...
                if ($userRight['add_files_deal']) { ?>
                    <li class="active"><?php echo CHtml::link('Сделки', Yii::app()->createUrl("page/document_deal_page")); ?></li>
                <? }
...
<main class="content full2" role="main">
    <?php $this->renderPartial('_documents_deals_search', array('documents' => $documents, 'user' => $user, 'userRight' => $userRight)); ?>

    <div class="box-gray">
        <div class="box-gray__body no-border bottom_margin">
            <?php
            if (count($documentDeal->data) == 0) { ?>
                <?
            }
            $this->widget('zii.widgets.grid.CGridView', array(
                'dataProvider' => $documentDeal,
                'cssFile' => '',
                    ...
                    array(
                        ...
                        'value' => function ($data) {
                            return
                                ...
                                CHtml::link($data->file->name, Yii::app()->createUrl("page/get_file_deal", ['id' => $data->id]), ['target' => '_blank']) .
                                '</div></div>';
                        }
                    ),
                    array(
                        ...
                        'type' => 'raw',
                        'value' => function ($data) {
                            return CHtml::link($data->deal->text, Yii::app()->createUrl("page/edit_deal", array("id" => $data->deal->id)));
                        }
                    ),
                    array(
                        'name' => 'date_upload',
...
<script>
    function delDocument(id) {
        if (confirm('Вы дествительно хотите удалить документ?')) {
            document.location.href = '/page/deal_document_delete?id=' + id + '&return=table';
        }
    }
</script>