<?php $this->pageTitle = 'Контакты | Новый фильтр'; ?>
                                                    ...
                                                    <ul>
                                                        <? foreach ($allLabels as $label) { ?>
                                                            <li id="labelLi <? echo $label->id ?>" class="labelLi"
                                                                ...
                                                                <?
                                                                echo CHtml::checkBox("labels_$label->id]", false, [
        ...
        <div class="box-gray__body">
            <div class="box-gray__form">
                <div class="form-group">
                    ...
                    <?echo CHtml::textField('nameFilter', '', ['class' => 'form-control']);?>
                ...
                <div class="form-group <?echo $user->roles[0]->name === 'manager' ? 'hide' : '' ?>">
                    ...
                    echo CHtml::dropDownList('responsible', 'all', $responsible_options, array('class' => 'styled permis editable typeAccess', 'name' => 'type'));
                   ?>
                </div>

                <div class="form-group">
                    <label class="label">Количество контактов: <div class="requiredAddField">*</div></label>
                    <?echo CHtml::textField('pageSize', 30, ['class' => 'form-control']);?>
                    ...
                    <div class="row-input" id="colorSelect">
                        <div class="jq-selectbox__select color-select"
                            ...
                            <div class="color-block"
                                 style="background-color: <? echo $this->filterColors[$classNameColorDefault] ?>">
                                <input type="text" id="classNameId" value="<? echo $classNameColorDefault ?>" class="hide">
                            </div>
                ...
                <div style="display: flex;line-height: 22px;margin-bottom: 2px;">
                    <?echo CHtml::checkBox('isDefault', false, ['class' => 'form-control_anketa']);?>
...
<script type="module">
    ...
    var blockFields = {
        'left': {
            ...
            'sourceAdditionalFields': <?echo json_encode($sectionFields)?>,
            'clientInfo': {},
            'additionalFields': {},
        },
    ...
    var blockLabelsList = <?echo json_encode($filterLabels)?>;
     ...
     window.changeLabel = function(labelId) {
        ...
        if ($('#checkbox' + labelId).is(':checked')) {
            ...
            elem.removeClass('deleted');
            elem.addClass('added');
            var labelDIv = '<div id="blockElem' + labelId + '" class="custom-label pointer" style="background-color: ' + blockLabelsList[labelId].color + '; color: ' + blockLabelsList[labelId].color_text + '; ">' + blockLabelsList[labelId].name + '</div>';
    ...
    window.saveFilter = function() {
        ...
        let data = {
            ...
            clientFiltersLabels: labelIds,
        };

        filterSaveBtnNode.classList.add('hide-important');
        let preloaderNode = document.getElementById('preloader');
        preloaderNode.classList.remove('hide-important');

        $.ajax({
            url: '/page/addFilterForClients',
            method: 'post',
            dataType: 'html',
            data: data,
            success: function(data) {
                ...
                if (response.status === 'success') {
                    document.location.replace('/page/clients_filters_edit?filterId='+ response.values.id + '&isAddFilter=true');
                } else {
